...
Public Class QLSanPham
    ...
    Private Sub Xuat_ThongTin()
        Dim lenh As String
        'Khai báo câu lệnh truy vấn dùng để đọc bảng SanPham
        lenh = "select * from tblSanPham"
        ...
        Dim dttable As New DataTable("tblSanPham")
        ...
        With Me.DataGridView1
            ...
            .Columns(2).Width = 150
            .Columns(2).HeaderText = "Loại"
            .Columns(3).Width = 100
            .Columns(3).HeaderText = "Giá"
            .Columns(4).Width = 150
            .Columns(4).HeaderText = "Số lượng còn"
        End With
    ...
    Private Sub xuat()
        ...
        tbTen.Text = danh_sach.Current("TenSP")
        tbLoai.Text = danh_sach.Current("Loai")
        tbSL.Text = danh_sach.Current("SoLuongCon")
        tbGia.Text = danh_sach.Current("Gia")
    End Sub
    ...
    Private Sub btLuu_Click(sender As Object, e As EventArgs) Handles btLuu.Click
        If MsgBox("Bạn có muốn lưu không?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Lưu") = MsgBoxResult.Yes Then
            If TbMsp.Text = "" Or tbTen.Text = "" Or tbLoai.Text = "" Or
                 tbSL.Text = "" Or tbGia.Text = "" Then
                MsgBox("Chưa nhập giá trị!!!")
            Else
                lenh = "insert into tblSanPham(MSP,TenSP,Loai,Gia,SoLuongCon) values('" &
               TbMsp.Text & "', N'" & tbTen.Text & "', N'" & tbLoai.Text & "', '" & tbGia.Text & "', '" & tbSL.Text & "')"
                Dim cmd As New OleDbCommand(lenh, Con)
                Con.Open()
                cmd.ExecuteNonQuery()
                Con.Close()
    ...
    Private Sub btSua_Click(sender As Object, e As EventArgs) Handles btSua.Click
        If MsgBox("Bạn có muốn sửa không ? ", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Sửa ") = MsgBoxResult.Yes Then
            ...
            Else
                lenh = "Update tblSanPham set MSP = '" & TbMsp.Text & "',
                                         TenSP    = N'" & tbTen.Text & "',
                                         Loai     = N'" & tbLoai.Text & "',
                                         Gia      = '" & tbGia.Text & "',
                                         SoLuongCon = '" & tbSL.Text & "' where MSP = '" & Trim(TbMsp.Text) & "'"
                Dim cmd As New OleDbCommand(lenh, Con)
                Con.Open()
                cmd.ExecuteNonQuery()
                Con.Close()
    ...
    Private Sub btXoa_Click(sender As Object, e As EventArgs) Handles btXoa.Click
        If MsgBox("Bạn có muốn xóa không ?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Xóa") = MsgBoxResult.Yes Then
            ...
            Else
                lenh = " Delete from tblSanPham where MSP = '" & TbMsp.Text & "' and MSP = '" & Trim(TbMsp.Text) & "'"
                Dim cmd As New OleDbCommand(lenh, Con)
                Con.Open()
                cmd.ExecuteNonQuery()
                Con.Close()
